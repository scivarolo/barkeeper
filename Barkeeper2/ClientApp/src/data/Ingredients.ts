...
export function useAllIngredientsLegacy() {
    return useQuery<Ingredient[], Error>('ingredients-all', () =>
        API.GET<Ingredient[]>('api/v1/ingredients')
    )
}
...
export function useAllIngredients() {
    ...
    `
    return useQuery<any, Error, Ingredient[]>(
        ['ingredients', 'all'],
        () => API.GraphQLQuery<any>(query),
        {
...
export function useSaveIngredient(onClose?: any) {
    ...
    return useMutation<Ingredient, string, Pick<Ingredient, 'name' | 'liquid'>>(
        (input) => API.GraphQLMutation<Ingredient>(mutation, { input }),
        {
            onSuccess: () => {
